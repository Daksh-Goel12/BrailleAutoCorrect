...
# def word_to_braille(word: str) -> str:
#     return ''.join(braille_map.get(c.lower(), '000000') for c in word)
...
#     input_braille = word_to_braille(input_word)
#     suggestions: List[Tuple[int, str]] = []

#     for word in dictionary:
#         dist = levenshtein(input_braille, word_to_braille(word))
#         suggestions.append((dist, word))

#     suggestions.sort()
#     best = suggestions[0][1]
#     remember_choice(input_word, best, memory_file)
#     return best

# def remember_choice(typed: str, corrected: str, memory_file="memory.json"):
#     memory = {}
#     if os.path.exists(memory_file):
#         with open(memory_file, "r") as f:
#             memory = json.load(f)
#     memory[typed] = corrected
#     with open(memory_file, "w") as f:
#         json.dump(memory, f, indent=2)

# if __name__ == "__main__":
#     print("Braille Auto-Correct System (CLI)")
#     dictionary = load_dictionary()
#     user_input = input("Enter a Braille word (QWERTY-style): ").strip()
#     print("Suggested correction:", suggest_word(user_input, dictionary))
...
class BrailleAutoCorrect:
    ...
    # def suggest_words(self, input_word: str, max_suggestions: int = 5, max_distance: int = 4) -> List[Tuple[str, int, float]]:
    #     """
    #     Suggest words with confidence scores
    #     Returns: List of (word, distance, confidence_score)
    #     """
    #     # Check if we've seen this exact correction before
    #     if input_word.lower() in self.user_corrections:
    #         learned_word = self.user_corrections[input_word.lower()]
    ...
    #     # Optimization: Check similar length words first
    ...
    #                     # Calculate confidence (higher for exact matches and learned words)
    #                     confidence = 1.0 - (distance / max(len(input_braille), len(word_braille)))
                        
    #                     # Boost confidence for previously learned corrections
    #                     if word in self.user_corrections.values():
    #                         confidence += 0.1
                        
    #                     suggestions.append((word, distance, confidence))
        
    #     # Sort by distance first, then by confidence
    ...
    #     input_len = len(input_braille)
    #
    #     # Set adaptive max distance based on input length
    ...
    #     suggestions.sort(key=lambda x: (x[1], -x[2]))
    #
    #     # Ensure at least 1 fallback if nothing meets distance
    #     if not suggestions:
